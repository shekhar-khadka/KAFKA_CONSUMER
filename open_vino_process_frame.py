...
# run_object_detection(source=VIDEO_SOURCE, flip=True, use_popup=False, model=det_ov_model, device="AUTO")
# Main processing function to run object detection.
def run_object_detection(source,core,label_map, fps,flip, use_popup, model, device):
    skip_first_frames = 0
    if device != "CPU":
    ...
    try:
        ...
        if scale < 1:
            frame = cv2.resize(
                ...
                fx=scale,
                fy=scale,
                interpolation=cv2.frameINTER_AREA,
            )
        # Get the results.
        input_image = np.array(source)
...
def initialize(VIDEO_SOURCE,fps):
    ...
    # else:
    #     VIDEO_SOURCE = 'v2.mp4'
    run_object_detection(source=VIDEO_SOURCE,core=core,fps=fps,label_map=label_map, flip=True, use_popup=False, model=det_ov_model, device="AUTO")
